...
1. Use Unity MCP to confirm the correct ready Editor and saved scene. Inspect **Airball Arena VR > Desktop XR > Show Local Status**.
2. Run **Airball Arena VR > Desktop XR > Play in Simulator** (plain Play uses the headset runtime), then query `openxr_get_session_info`. Test input only after a fresh FOCUSED session; an offline tool list is insufficient.
