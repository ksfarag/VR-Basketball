...
pwsh.exe -NoProfile -File "%~dp0Deploy-Quest.ps1" %*
exit /b %errorlevel%
:standard
if not exist "%ProgramFiles%\PowerShell\7\pwsh.exe" goto missing
"%ProgramFiles%\PowerShell\7\pwsh.exe" -NoProfile -File "%~dp0Deploy-Quest.ps1" %*
exit /b %errorlevel%
:configured
if not exist "%PWSH_EXE%" goto missing
"%PWSH_EXE%" -NoProfile -File "%~dp0Deploy-Quest.ps1" %*
exit /b %errorlevel%
:missing
