...
Select the Simulator in Meta Core's own settings so that its toolbar Simulator toggle (**Meta > Meta XR Simulator > Activate**) works. With Unity out of Play mode, choose **Airball Arena VR > Desktop XR > Configure Experimental Operator** and select the Operator JSON. Register the proxy in Claude Code using its actual path:
